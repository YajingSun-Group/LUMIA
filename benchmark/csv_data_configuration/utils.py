...
def split_dataset(args, dataset):
    ...
    args : dict
        ...
        - split: str
            - 'predefine': Use predefined split from a column in CSV
            - 'random': Random split
            - 'scaffold_smiles': Split based on scaffold
            - 'scaffold_decompose': Split based on scaffold decomposition
        - split_ratio: List[float]
            Division of dataset into training, validation and test sets
        - split_column: str, optional
            Column name containing split information when split=='predefine'
    dataset : DGLDataset
    ...
    train_set, val_set, test_set : 
        Split datasets
    """
    if args['split'] == 'predefine':
        assert 'split_column' in args, 'Must specify split_column for predefine split'
        train_indices = []
        val_indices = []
        test_indices = []
        
        # 获取划分信息
        split_info = args['df'][args['split_column']]
        for i, split in enumerate(split_info):
            if split.lower() == 'train':
                train_indices.append(i)
            elif split.lower() in ['valid', 'val', 'validate']:
