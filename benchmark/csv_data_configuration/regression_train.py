...
def run_an_eval_epoch_with_prediction(args, model, data_loader):
    """运行评估并返回预测结果"""
    model.eval()
    eval_meter = Meter()
    smiles_list = []
    ...
    return np.mean(eval_meter.compute_metric(args['metric'])), smiles_list, predictions, labels
...
def main(args, exp_config, train_set, val_set, test_set):
    ...
    test_score, smiles, predictions, labels = run_an_eval_epoch_with_prediction(args, model, test_loader)
    ...
    if args['task_names'] is None:
        task_names = [f'task_{i}' for i in range(args['n_tasks'])]
    else:
    ...
    # 保存预测结果
    results_df.to_csv(os.path.join(args['trial_path'], 'test_predictions.csv'), index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--split',
                        choices=['predefine', 'scaffold_decompose', 'scaffold_smiles', 'random'],
                        default='scaffold_smiles',
                        help='Dataset splitting method (default: scaffold_smiles). For predefine split, '
                             'use predefine and specify split_column.')
    parser.add_argument('-split-column', type=str, default=None,
                        help='Column name containing split information (train/valid/test) '
                             'when using predefine split')
    ...
    for task in args['task_names']:
        ...
        if args['num_evals'] is not None:
            ...
            trial_path = os.path.join(args['result_path'], '1')

        copyfile(os.path.join(trial_path, 'model.pth'), 
                os.path.join(args['result_path'], 'model.pth'))
        copyfile(os.path.join(trial_path, 'configure.json'), 
                os.path.join(args['result_path'], 'configure.json'))
        copyfile(os.path.join(trial_path, 'eval.txt'), 
                os.path.join(args['result_path'], 'eval.txt'))
