...
class Finetuner(object):
    def __init__(self, dataset, config, logger,is_predict=False,normalize_path=None):
        self.args = config
        ...
        if self.args.normalize:
            ...
            if self.is_predict:
                self.normalize.load_my_state_dict(normalize_path)
    ...
    def build_model(self):
        ...
        if self.args.resume_from is not None:
            checkpoint = torch.load(os.path.join(self.args.resume_from, 'checkpoints', 'early_stop.pth'))
    ...
    def predict(self,data_loader,tag='predict'):
        """预测函数"""
        ...
        with torch.no_grad():
            ...
            for bn, batch_data in progress_bar:
                ...
                # 如果使用了normalize，需要反归一化预测结果
                if self.normalize is not None:
                    preds = self.normalize.denorm(preds.cpu())
                else:
                    preds = preds.cpu()
        ...
        for i, task in enumerate(self.args.task_list):
            results_dict[f'{task}_pred'] = predictions[:, i]
        ...
        self.logger.info(f"预测结果已保存至: {results_save_path}")
