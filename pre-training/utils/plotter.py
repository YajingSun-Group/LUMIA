...
def plot_fill_a_ring(mol,ring_atoms,ring_bonds,color_id = 1):
    ...
    return svg
def plot_with_color2(mol, atom_list1, atom_list2=None, bond_list1=None, bond_list2=None, lineWidth=2, atomrads=0.4, widthmults=2, fontSize=16, figsize=(500,500), color_id1=1, color_id2=2):
    # Thanks for MolSHAP: https://github.com/tiantz17/MolSHAP/blob/master/utils.py
    
    # 如果没有提供bond_list1和bond_list2，自动生成它们
    if bond_list1 is None:
        bond_list1 = []
        for bond in mol.GetBonds():
            if bond.GetBeginAtom().GetIdx() in atom_list1 and bond.GetEndAtom().GetIdx() in atom_list1:
                bond_list1.append(bond.GetIdx())
    
    if bond_list2 is None and atom_list2 is not None:
        bond_list2 = []
        for bond in mol.GetBonds():
            if bond.GetBeginAtom().GetIdx() in atom_list2 and bond.GetEndAtom().GetIdx() in atom_list2:
                bond_list2.append(bond.GetIdx())
    ...
    # 标准化颜色到 [0,1] 范围
    color1 = tuple(y/255 for y in colors[color_id1])
    color2 = tuple(y/255 for y in colors[color_id2]) if atom_list2 else color1

    # 查找 atom_list1 和 atom_list2 中的环
    Chem.GetSSSR(mol)
    rinfo  = mol.GetRingInfo()
    
    list_r1 = []
    for aring in rinfo.AtomRings():
        overlap = np.intersect1d(aring, atom_list1)
        if len(overlap) == len(aring):
            list_r1.append(aring)
    
    list_r2 = []
    if atom_list2:
        for aring in rinfo.AtomRings():
            overlap = np.intersect1d(aring, atom_list2)
            if len(overlap) == len(aring):
                list_r2.append(aring)

    # 创建原子和键的颜色字典
    dict_color1 = {int(idx): [color1] for i in range(len(list_r1)) for idx in list_r1[i]}
    dict_color2 = {int(idx): [color2] for i in range(len(list_r2)) for idx in list_r2[i]} if atom_list2 else {}

    dict_b_color1 = {int(b): [color1] for b in bond_list1}
    dict_b_color2 = {int(b): [color2] for b in bond_list2} if bond_list2 else {}

    atomrads_ring1 = {i: atomrads for i in dict_color1}
    atomrads_ring2 = {i: atomrads for i in dict_color2}

    widthmults_ring1 = {i: widthmults for i in dict_b_color1}
    widthmults_ring2 = {i: widthmults for i in dict_b_color2}
    ...
    # 在绘制多边形之前，先绘制分子以初始化缩放和坐标
    d2d.DrawMolecule(mol)

    # 绘制环
    for aring in list_r1:
        ps = [Geometry.Point2D(conf.GetAtomPosition(aidx)) for aidx in aring]
        d2d.SetFillPolys(True)
        d2d.SetColour(color1)
        d2d.DrawPolygon(ps)
    
    if atom_list2:
        for aring in list_r2:
            ps = [Geometry.Point2D(conf.GetAtomPosition(aidx)) for aidx in aring]
            d2d.SetFillPolys(True)
            d2d.SetColour(color2)
            d2d.DrawPolygon(ps)

    # 绘制原子和键，分别处理 atom_list1、bond_list1 和 atom_list2、bond_list2
    dict_color_all = {**dict_color1, **dict_color2}
    dict_b_color_all = {**dict_b_color1, **dict_b_color2}
    
    atomrads_all = {**atomrads_ring1, **atomrads_ring2}
    widthmults_all = {**widthmults_ring1, **widthmults_ring2}
    
    d2d.DrawMoleculeWithHighlights(mol, "", dict_color_all, dict_b_color_all, atomrads_all, widthmults_all)
    d2d.FinishDrawing()
