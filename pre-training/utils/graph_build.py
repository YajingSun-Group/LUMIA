...
# Thanks for the code from dgllife: https://github.com/awslabs/dgl-lifesci
...
def mol_to_graph(mol,
                 graph_constructor, node_featurizer, edge_featurizer,
                 canonical_atom_order, 
                 mask_nodes_indices=[],
                 mask_edges_indices=[],
                 explicit_hydrogens=False, num_virtual_nodes=0):
    ...
    if node_featurizer is not None:
        g.ndata.update(node_featurizer(mol,mask_nodes_indices))

    if edge_featurizer is not None:
        g.edata.update(edge_featurizer(mol,mask_edges_indices))
...
def mol_to_bigraph(mol,
    ...
    return mol_to_graph(mol, 
                        mask_nodes_indices=mask_nodes_indices,
                        mask_edges_indices=mask_edges_indices,
                        graph_constructor=partial(construct_bigraph_from_mol, add_self_loop=add_self_loop),
...
def smiles_to_bigraph(smiles,
    ...
    return mol_to_bigraph(mol,
                           mask_nodes_indices,
                           mask_edges_indices,
                           add_self_loop, node_featurizer, edge_featurizer,
...
class SMILESToBigraph(ToGraph):
    ...
    def __call__(self, smiles,mask_nodes_indices=[],mask_edges_indices=[]):
        """Construct graph for the molecule and featurize it.
