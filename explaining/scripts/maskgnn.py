...
def collate_molgraphs(data):
    smiles, g_rgcn, labels, smask, sub_name,additional_features = map(list, zip(*data))
    rgcn_bg = dgl.batch(g_rgcn)
    labels = th.tensor(labels)
    return smiles, rgcn_bg, labels, smask, sub_name,additional_features
...
def run_an_eval_epoch(args, model, data_loader, loss_criterion, out_path, seed=0,normalize=None,embed_molecular_features=False):
    model.eval()
    total_loss = 0
    n_mol = 0  # 添加n_mol初始化
    smiles_list = []  # 添加列表初始化
    ...
    with th.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            smiles, rgcn_bg, labels, smask_idx, sub_name,additional_features = batch_data
            rgcn_bg = rgcn_bg.to(args['device'])
            labels = labels.unsqueeze(dim=1).float().to(args['device'])
            # 添加对additional_features的检查
            if embed_molecular_features:
                additional_features = th.tensor(additional_features).float().to(args['device'])
            else:
            ...
            preds, weight,_ = model(rgcn_bg, rgcn_node_feats, rgcn_edge_feats, smask_feats,additional_features)
            loss = (loss_criterion(preds, labels)).mean()
    ...
    if normalize is not None:
        y_pred = normalize.denorm(y_pred)
        y_true = normalize.denorm(y_true)
    ...
    if out_path is not None:
        smask_idx_list = np.array(smask_idx_list,dtype=object)
