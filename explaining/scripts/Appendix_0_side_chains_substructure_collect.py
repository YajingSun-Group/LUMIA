...
def main():
    ...
    side_chains_information = []
    smask_npy = np.load('../data/graph_data/hr_smask_for_side_chain.npy',allow_pickle=True)
    ...
    for i in [2]:
        ...
        for input_smiles in tqdm(smiles_list,desc=f'cv_{i}'):
            ...
            substructure_dir = PartitionMol(input_smiles,bone_selection="lgcs")
            attribution_i = list(attribution_data[attribution_data['smiles'] == input_smiles]['attribution'])
            attribution_i = attribution_i[:len(substructure_dir['side_chains'])]


            if len(substructure_dir['side_chains']) != len(attribution_i):
                print(f'{input_smiles} 的 side_chains 数量与 attribution 数量不一致')
                print(len(substructure_dir['side_chains']), len(attribution_i))
                break
            
            for num_substructure, side_chain in enumerate(substructure_dir['side_chains']):
                substructure = side_chain['atom_idx']
                is_valid = 0
                sub_smiles = extract_substructure(input_smiles, substructure, return_format='smiles')
                sub_smarts = extract_substructure(input_smiles, substructure, return_format='smarts')
                if sub_smiles is not None and sub_smiles != input_smiles:
                ...
                side_chains_information.append([input_smiles, substructure, sub_smiles, sub_smarts, is_valid, attribution])
        
        side_chains_information_df = pd.DataFrame(side_chains_information, columns=['smiles','substructure_atom_indices','substructure_smiles','substructure_smarts','is_valid','attribution'])
        side_chains_information_df.to_csv(f'../Figure-4a-data/side-chains/side_chains_information_attribution_cv_{i}.csv', index=False)
