...
from utils.featurizer import CanonicalAtomFeaturizer
import dgl
...
def return_fg_hit_atom(smiles, fg_name_list, fg_with_ca_list, fg_without_ca_list):
    ...
    # sort function split atom by atom number
    sorted_all_hit_fg_at = sorted(all_hit_fg_at,
                                  key=lambda fg: len(fg),
                                  reverse=True)
    # remove small function split (wrongly matched), they are part of other big function groups
    remain_fg_list = []
        ...
        if fg not in remain_fg_list:
            if len(remain_fg_list) == 0:
                ...
                if i == len(remain_fg_list):
                    remain_fg_list.append(fg)
    # wash the hit function split atom by using the remained fg, remove the small wrongly matched fg
    hit_at_wash = []
    hit_fg_name_wash = []
...
def construct_RGCN_mol_graph_from_smiles(smiles, smask, atom_featurizer):
    # Add nodes
    mol = MolFromSmiles(smiles)
    num_atoms = mol.GetNumAtoms()
    smask_list = []
    atoms_feature_all = atom_featurizer(mol)
    ...
    # Create graph using dgl.graph()
    g = dgl.graph((th.tensor(src_list), th.tensor(dst_list)), num_nodes=num_atoms)
    
        # Add node and edge features - 使用clone().detach()
    if isinstance(atoms_feature_all['node'], th.Tensor):
        node_features = atoms_feature_all['node']
    else:
        node_features = th.from_numpy(atoms_feature_all['node'])
    
    # Add node and edge features - 使用clone().detach()
    g.ndata["node"] = node_features.clone().detach()
...
def build_mol_graph_data(dataset_smiles, labels_name, smiles_name):
    ...
    split_index = dataset_smiles['split']
    smilesList = dataset_smiles[smiles_name]
    molecule_number = len(smilesList)
    atom_featurizer = CanonicalAtomFeaturizer(atom_data_field='node')
    for i, smiles in enumerate(smilesList):
        try:
            g_rgcn = construct_RGCN_mol_graph_from_smiles(smiles, smask=[], atom_featurizer=atom_featurizer)
...
def build_mol_graph_data_for_brics(dataset_smiles, labels_name, smiles_name):
    ...
    for i, smiles in enumerate(smilesList):
        ...
        for j, smask_i in enumerate(smask):
            try:
                g_rgcn = construct_RGCN_mol_graph_from_smiles(smiles, smask=smask_i, atom_featurizer=atom_featurizer)
...
def build_mol_graph_data_for_skelton(dataset_smiles, labels_name, smiles_name):
    dataset_gnn = []
    failed_molecule = []
    labels = dataset_smiles[labels_name]
    split_index = dataset_smiles['split']
    ...
    for i, smiles in enumerate(smilesList):
        ...
        atom_mask = []
        substructure_mask = []
        # skeleton只有一个，直接取atom_idx
        atom_mask = leaves_structure['skeleton']['atom_idx'] 
        substructure_mask.append(atom_mask)
        # smask is atom_mask, because there is only one skeleton in the molecule
...
def build_mol_graph_data_for_side_chain(dataset_smiles, labels_name, smiles_name):
    dataset_gnn = []
    failed_molecule = []
    labels = dataset_smiles[labels_name]
    split_index = dataset_smiles['split']
    ...
    for i, smiles in enumerate(smilesList):
        ...
        atom_mask = []
        side_chain_mask = []
        # side_chains可能为空列表，也可能有1个或多个字典元素
        if len(leaves_structure['side_chains']) > 0:
            for substructure in leaves_structure['side_chains']:
                atom_mask = atom_mask + substructure['atom_idx']
                side_chain_mask.append(substructure['atom_idx'])
        smask = side_chain_mask
        for j, smask_i in enumerate(smask):
            try:
                g_rgcn = construct_RGCN_mol_graph_from_smiles(smiles, smask=smask_i, atom_featurizer=atom_featurizer)
                molecule = [smiles, g_rgcn, labels.loc[i], split_index.loc[i], smask_i]
...
def build_mol_graph_data_for_fg(dataset_smiles, labels_name, smiles_name, task_name):
    ...
    # build mol graph for function split
    dataset_gnn = []
    failed_molecule = []
    labels = dataset_smiles[labels_name]
    split_index = dataset_smiles['split']
    ...
    for i, smiles in enumerate(smilesList):
        ...
        for j, fg_mask_j in enumerate(fg_mask):
            try:
                g_rgcn = construct_RGCN_mol_graph_from_smiles(smiles, smask=fg_mask_j, atom_featurizer=atom_featurizer)
...
def build_mol_graph_data_for_brics_emerge(dataset_smiles, labels_name, smiles_name):
    ...
    for i, smiles in enumerate(smilesList):
        ...
        for j, pair_index in enumerate(all_pair_index):
            ...
            try:
                g_rgcn_1 = construct_RGCN_mol_graph_from_smiles(smiles, smask=pair_sub[0], atom_featurizer=atom_featurizer)
                molecule_1 = [smiles, g_rgcn_1, labels.loc[i], split_index.loc[i], pair_sub[0], 'emerge_{}'.format(j)]
                dataset_gnn.append(molecule_1)
                g_rgcn_2 = construct_RGCN_mol_graph_from_smiles(smiles, smask=pair_sub[1], atom_featurizer=atom_featurizer)
...
def built_mol_graph_data_and_save(
        ...
        save_g_for_skelton_path='No_name_mol_graph_for_skelton.bin',
        save_g_smask_for_skelton_path='No_name_mol_graph_smask_for_skelton.npy',
        save_g_group_for_skelton_path='No_name_mol_graph_group_for_skelton.csv',
        save_g_for_side_chain_path='No_name_mol_graph_for_side_chain.bin',
        save_g_smask_for_side_chain_path='No_name_mol_graph_smask_for_side_chain.npy',
        save_g_group_for_side_chain_path='No_name_mol_graph_group_for_side_chain.csv'
):
    ...
    split_index_pd = pd.DataFrame(columns=['smiles', 'split'])
    split_index_pd.smiles = smiles
    split_index_pd.split = split_index
    ...
    # build data for skelton
    # data_set_gnn_for_skelton = build_mol_graph_data_for_skelton(dataset_smiles=data_origin, labels_name=labels_name,
    #                                                 smiles_name=smiles_name)
    # smiles, g_rgcn, labels, split_index, smask = map(list, zip(*data_set_gnn_for_skelton))
    # graph_labels = {'labels': th.tensor(labels)
    #                 }
    # split_index_pd = pd.DataFrame(columns=['smiles', 'split'])
    # split_index_pd.smiles = smiles
    # split_index_pd.split = split_index
    # split_index_pd.to_csv(save_g_group_for_skelton_path, index=False, columns=None)
    # smask_np = np.array(smask, dtype=object)
    # np.save(save_g_smask_for_skelton_path, smask_np)
    # print('Molecules graph for skelton is saved!')
    # save_graphs(save_g_for_skelton_path, g_rgcn, graph_labels)
    
    # build data for side chain
    # data_set_gnn_for_side_chain = build_mol_graph_data_for_side_chain(dataset_smiles=data_origin, labels_name=labels_name,
    #                                                 smiles_name=smiles_name)
    # smiles, g_rgcn, labels, split_index, smask = map(list, zip(*data_set_gnn_for_side_chain))
    # graph_labels = {'labels': th.tensor(labels)
    #                 }
    # split_index_pd = pd.DataFrame(columns=['smiles', 'split'])
    # split_index_pd.smiles = smiles
    # split_index_pd.split = split_index
    # split_index_pd.to_csv(save_g_group_for_side_chain_path, index=False, columns=None)
    # smask_np = np.array(smask, dtype=object)
    # np.save(save_g_smask_for_side_chain_path, smask_np)
    # print('Molecules graph for side chain is saved!')
    # save_graphs(save_g_for_side_chain_path, g_rgcn, graph_labels)
    ...
    # graph_labels = {'labels': th.tensor(labels)
    #                 }
    # split_index_pd = pd.DataFrame(columns=['smiles', 'split', 'sub_name'])
    # split_index_pd.smiles = smiles
    # split_index_pd.split = split_index
    ...
    # print('Molecules graph for function split is saved!')
...
def load_graph_from_csv_bin_for_splited(
        ...
        seed=2022,
        normalize=None,
        additional_features_path=None
):

    
    data = pd.read_csv(group_path)
    if additional_features_path is not None:
        additional_features = pd.read_csv(additional_features_path)
        # 使用merge和repeat来加速处理
        # 先获取每个SMILES的重复次数
        smiles_counts = data['smiles'].value_counts().reset_index()
        smiles_counts.columns = ['smiles', 'count']
        print('Counting smiles...')
        
        # 将additional_features与smiles_counts合并
        merged = pd.merge(smiles_counts, additional_features, on='smiles', how='left')
        print('Merging smiles and additional_features...')
        # 使用repeat展开数据
        expanded_features = merged.loc[merged.index.repeat(merged['count'])].reset_index(drop=True)
        print('Expanding data...')
        
        # 删除不需要的列并确保所有列都是数值类型
        additional_features = expanded_features.drop(['count', 'smiles'], axis=1)
        # 转换为数值类型
        additional_features = additional_features.apply(pd.to_numeric, errors='coerce')
        additional_features = additional_features.fillna(0)  # 将任何NaN值填充为0
        additional_features = additional_features.to_numpy()
        
        print('additional_features is loaded!')
        print(f'additional_features shape: {additional_features.shape}')
    else:
        additional_features = None
        
    smiles = data.smiles.values
    split = data.split.to_list()
    # load substructure name
    if 'sub_name' in data.columns.tolist():
        sub_name = data['sub_name']
    else:
        sub_name = ['noname' for x in split]
    ...
    if normalize is not None:
        labels = normalize.norm(labels)
        print('labels is normalized!')
    # load smask
    if smask_path is None:
        smask = [-1 for x in range(len(split))]
    else:
    ...
    val_index = []
    test_index = []
    for index, group_index in enumerate(split):
    ...
    for i in train_index:
        if additional_features is not None:
            molecule = [smiles[i], homog[i], labels[i], smask[i], sub_name[i], additional_features[i]]
        else:
            molecule = [smiles[i], homog[i], labels[i], smask[i], sub_name[i], None]
